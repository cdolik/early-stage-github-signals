...
try:
    ...
    if not dotenv_loaded:
        ...
        if dotenv_loaded:
            ...
            print("Tip: Run 'make setup' or copy .env.example to .env and add your GitHub token")
except ImportError:
    print("Warning: python-dotenv not installed. Installing dependencies with 'pip install -r requirements.txt' is recommended.")
...
def validate_github_token(token):
    ...
    try:
        ...
        return True, f"Token valid for user: {username} (Rate limit: {remaining_calls} API calls remaining)"
    except Exception as e:
        error_message = str(e)
        # Don't include the token in error messages
        sanitized_error = error_message.replace(token, "[TOKEN]") if token in error_message else error_message
...
def validate_data(scored_repos: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    ...
    for repo in scored_repos:
        ...
        name = repo.get('name') or (repo.get('full_name', '').split('/')[-1] if repo.get('full_name') else 'Unknown')
...
def main():
    ...
    if github_token:
        if args.debug:
            ...
            token_source = "command line" if args.github_token else "environment variable"
            logger.debug(f"Using GitHub token from {token_source} (starts with: {token_preview})")
        ...
        try:
            ...
            if is_valid:
                ...
                logger.info(f"GitHub API client initialized successfully: {validation_message}")
            else:
        ...
        logger.debug("2. Set it as an environment variable: export GITHUB_TOKEN=your_token")
    ...
    logger.info(f"Validated repository data: {len(validated_repos)} repositories ready for output")
